...
{
    internal class QuasarCrawlerHead : QuasarCrawler
    {
        public override string Texture => "TenebraeMod/NPCs/QuasarCrawler_Head";
        ...
        public override void CustomBehavior()
        {
            if (Main.netMode != NetmodeID.Server)
            {
                if (attackCooldown == 0)
                {
                    ...
                    laser = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, npc.velocity.X, npc.velocity.Y, ProjectileType<QuasarCrawlerLaser>(), 80, 2f, Main.myPlayer, 0, npc.whoAmI);
                }
                if (laser != -1)
                {
            ...
            if (attackCooldown != -30 || Main.tile[(int)(npc.Center.X / 16), (int)(npc.Center.Y / 16)].collisionType == 0)
                attackCooldown++;
        }
    }

    internal class QuasarCrawlerBody : QuasarCrawler
    {
        public override string Texture => "TenebraeMod/NPCs/QuasarCrawler_Body";
    ...
    internal class QuasarCrawlerTail : QuasarCrawler
    {

        public override string Texture => "TenebraeMod/NPCs/QuasarCrawler_Tail";
    ...
    public abstract class QuasarCrawler : Worm
    {
        ...
        public override void Init()
        {
            maxLength = 20 - 2;
            minLength = 20 - 2;
            tailType = NPCType<QuasarCrawlerTail>();
            bodyType = NPCType<QuasarCrawlerBody>();
            headType = NPCType<QuasarCrawlerHead>();
            speed = 10f;
            turnSpeed = 0.03f;
        }
    }
    ...
    public abstract class Worm : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (npc.ai[1] > 0f && npc.ai[1] < (float)Main.npc.Length)
            {
                ...
                {
                    vector18 = npc.Center;
                    num191 = Main.npc[(int)npc.ai[1]].Center.X - npc.Center.X + (0.5f * npc.width * (new Vector2(1, 0)).RotatedBy(Main.npc[(int)npc.ai[1]].rotation)).X;
                    num192 = Main.npc[(int)npc.ai[1]].Center.Y - npc.Center.Y + (0.5f * npc.height * (new Vector2(1, 0)).RotatedBy(Main.npc[(int)npc.ai[1]].rotation)).Y;
                }
                catch
                {
                    npc.netUpdate = true;
                ...
                npc.rotation = (float)System.Math.Atan2((double)num192, (double)num191) + (float)Math.PI;
            ...
            else
            {
                ...
                else
                {
                    ...
                    bool flag21 = false;
                    if (npc.type == NPCID.WyvernHead || npc.type == NPCType<VileLeech>())
                    {
                ...
                npc.rotation = (float)System.Math.Atan2((double)npc.velocity.Y, (double)npc.velocity.X) + (float)Math.PI;
                if (head)
                {
                    if (flag18)
                    {
